...
char **com_n_args(char *inp) {
	...
	while(token != NULL){
		res[k]=(char *)malloc(sizeof(char) * C_MAX);
		strcpy(res[k++],token);
		token = strtok(NULL," ");
	}
	res[k] = (char *)malloc(sizeof(char) * C_MAX);
...
void pip_pls(char **inp){
	int pipefd[2];
	int m=0,j,r=0;
	char *cmd1 = (char *)malloc(sizeof(char)*C_MAX);
	char *cmd2 = (char *)malloc(sizeof(char)*C_MAX);
...
char *do_pip_stuff(char **inp){
	int i=0;
	char *new = (char *)malloc(sizeof(char)*(ARG_MAX*C_MAX));
	int k=0;
	do{
		if( strcmp(inp[i],">")==0 ){
			char dest[C_MAX];
...
void do_func() {
	...
	char *input2 = (char *)malloc(sizeof(char) * C_MAX);
	...
	if( strcmp(p_input, "piped") ==0 ) {
		...
		if(strcmp(*list, "hist") == 0) {
			char line[C_MAX];
			FILE *new = fopen(HIST, "ab+");
			while( fgets(line, C_MAX, new) != NULL ){
				printf("%s", line);
			}
			hist = fopen(HIST, "ab+");
			fprintf(hist, "hist\n");
		...
		if(strcmp(*list, "cd")==0) {
			cd_stack[pd] = malloc(sizeof(char *) * C_MAX);
			getcwd(cd_stack[pd], C_MAX);
			printf("\n---cd_stack[%d]=%s---\n", pd, cd_stack[pd]);
			pd++;
			chdir(list[1]);
			hist = fopen(HIST, "ab+");
			...
			char *python_command = (char *)malloc(sizeof(char) * (strlen(str1) + strlen(CWD) + strlen(str3) + 1));
			strcpy(python_command, str1);
			strcat(python_command, CWD);
			strcat(python_command, str3);
			printf("%s\n", python_command);
			...
			*/
			...
			fclose(hist);
			if((pid=fork()) == -1) {
				fprintf(stderr, "aweShell : can't fork : %s\n", strerror(errno));
			}
			else if(pid==0) {
				execvp(*list, list);
				fprintf(stderr, "aweShell : can't exec : %s\n", strerror(errno));
				exit(EX_DATAERR);
			}
			if( (pid=waitpid(pid, &status, 0)) < 0) {
				fprintf(stderr, "aweShell : waitpid error : %s\n", strerror(errno));
			}
		}
	}
}
int main(){
	...
	printf(KNRM "\n-----------------------------------------------------------------------------------------------------------------------------------------------------------------\n\n");
	while(1){
		char* path = (char *)malloc(sizeof(char) * C_MAX);
		getcwd(path, C_MAX);
